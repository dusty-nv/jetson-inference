...
To process test images with [`detectNet`](detectNet.h) and TensorRT, use the `detectnet-console` program.  `detectnet-console` accepts command-line arguments representing the path to the input image and path to the output image (with the bounding box overlays rendered).  Some test images are included with the repo:
