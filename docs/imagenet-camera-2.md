...
- optional `--camera` flag setting the camera device to use
	...
	- V4L2 USB cameras are used by specifying their `/dev/video` node (`/dev/video0`)
	- The default is to use MIPI CSI sensor 0 (`--camera=0`)
- `--width` and `--height` flags setting the camera resolution (default is `1280x720`)

You can combine the usage of these flags as needed.  Below are some scenarios for launching the program:
...
> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- TX1/TX2:  devkits include an onboard MIPI CSI sensor module (0V5693)<br/>
