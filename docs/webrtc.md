...
<br/>
<sup>Web App Frameworks</sup></s></p>
...
<p align="center"><img src="https://github.com/dusty-nv/jetson-inference/raw/dev/docs/images/webrtc-viewer.jpg" width="600"></p>
...
It's recommended to use HTTPS and SSL/TLS for transporting WebRTC streams and serving the webpages so that they are encrypted.  Also, browsers require HTTPS to enable the user's webcams from a PC.  To use HTTPS, first you need to generate a self-signed SSL certificate and key:
...
You can then use the videoOutput interface in your application's main loop to render frames just like you would have before like in these [examples](aux-streaming.md#source-code).
...
You can then use the videoSource interface in your application's main loop to capture video just like you would have before like in these [examples](aux-streaming.md#source-code).
