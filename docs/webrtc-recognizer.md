...
The `--data` argument sets the path where your dataset and models are stored under.  If you built jetson-inference from source, you should elect to [Install PyTorch](building-repo-2.md#installing-pytorch) (or run the `install-pytorch.sh` script again), as this example depends on PyTorch.  If you're using the jetson-inference container, PyTorch is already installed for you.
